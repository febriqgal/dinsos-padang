/* eslint-disable react-hooks/exhaustive-deps */
/* eslint-disable @next/next/no-img-element */
import { Button, Loading, Tooltip } from "@nextui-org/react";
import dayjs from "dayjs";
import "dayjs/locale/id";
import relativeTime from "dayjs/plugin/relativeTime";
import {
  collection,
  doc,
  getDocs,
  orderBy,
  query,
  updateDoc,
} from "firebase/firestore";
import Head from "next/head";
import Image from "next/image";
import { useRouter } from "next/router";
import { useEffect, useRef, useState } from "react";
import Layout from "../../components/layout";
import dibuat from "../../../public/dibuat.svg";
import penulis from "../../../public/penulis.svg";
import styles from "../../styles/Home.module.css";
import { db } from "@/db/firebase";
import { useUser } from "@/context/user";
export default function LayouUser() {
  const route = useRouter();
  const { email } = useUser();
  dayjs.locale("id");
  dayjs.extend(relativeTime);
  const snapshot = useRef(null);
  const [isLoading, setIsloading] = useState(true);
  const getDBFromFirestore = async () => {
    const querySnapshot = query(
      collection(db, "pengaduan"),
      orderBy("tanggal", "desc")
    );
    const gettt = await getDocs(querySnapshot);
    snapshot.current = gettt.docs;
    setTimeout(() => {
      setIsloading(false);
    }, 1000);
  };

  useEffect(() => {
    getDBFromFirestore();
  }, []);

  if (isLoading) {
    return (
      <div className={styles.center}>
        <Loading color={"currentColor"} />
      </div>
    );
  } else {
    const post = snapshot.current;
    const data = Object.values(post);
    return (
      <Layout title={"Pengaduan - "}>
        <Button
          onPress={() => {
            route.push("/pengaduan/tambah-pengaduan");
          }}
          className="bg-sky-900 m-auto mt-5"
        >
          Tambah Pengaduan
        </Button>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 py-5 px-5 gap-5">
          {data.map((e, i) => {
            const Data = e.data();
            return (
              <div
                key={i}
                className="mx-auto w-full rounded-xl shadow-lg bg-white max-w-sm"
              >
                <div className="overflow-clip rounded-t-lg rounded-br-3xl shadow-xl">
                  <img
                    onClick={async () => {
                      email != "admin@dinsos.com"
                        ? null
                        : route.push(`/pengaduan/${e.id}`);
                      const frankDocRef = doc(db, "pengaduan", `${e.id}`);
                      await updateDoc(frankDocRef, {
                        dilihat: Data.dilihat + 1,
                      });
                    }}
                    className="object-cover h-48 w-full hover:scale-110 duration-1000 hover:cursor-pointer"
                    src={
                      `https://firebasestorage.googleapis.com/v0/b/dinsos-padang.appspot.com/o/image%2Fpengaduan%2F${Data.gambar}?alt=media&token=8b0fd6b4-44b1-49dc-a17a-e352a250bf1a` ??
                      "https://firebasestorage.googleapis.com/v0/b/dinsos-padang.appspot.com/o/image%2Fpengaduan%2Fdinsos.png?alt=media&token=8b0fd6b4-44b1-49dc-a17a-e352a250bf1a"
                    }
                    alt={"#"}
                  />
                </div>
                <div className="px-5 py-5">
                  <div className="justify-between mb-4">
                    <h5 className="text-gray-900 text-xl font-bold">
                      <div className={styles.truncate2}>{Data.judul}</div>
                    </h5>
                    <div className="flex justify-between mt-1">
                      <div className="flex items-center gap-2">
                        <Image src={penulis} width={20} alt={"#"} />
                        <h5 className="text-gray-900 text-xs font-medium ">
                          {Data.penulis}
                        </h5>
                      </div>
                      <div className="flex items-center gap-2">
                        <Image src={dibuat} width={20} alt={"#"} />
                        <Tooltip shadow={true} content={Data.tanggal_pengaduan}>
                          <h5 className="text-gray-900 text-xs font-medium ">
                            {dayjs(Data.tanggal).fromNow()}
                          </h5>
                        </Tooltip>
                      </div>
                    </div>
                  </div>
                  <div className="text-gray-700  h-20 text-base">
                    <h1 className={styles.truncate3}>{Data.isi}</h1>
                  </div>
                </div>
              </div>
            );
          })}
        </div>
      </Layout>
    );
  }
}
